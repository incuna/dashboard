class RedmineRouter(object):
    ...
    def db_for_read(self, model, **hints):
        "Point all operations on redmine models to 'other'"
    ...
    def db_for_write(self, model, **hints):
        "Point all operations on redmine models to 'other'"
        if hasattr(model, '_meta') and model._meta.app_label == 'redmine':
            return 'redmine'
        return None

    def allow_relation(self, obj1, obj2, **hints):
        "Allow any relation if a model in redmine is involved"
        if obj1._meta.app_label == 'redmine' or obj2._meta.app_label == 'redmine':
            return True
        return None
