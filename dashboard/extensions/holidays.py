...
def _accepted_days(self, yearoffset=0):
    ...
    qs = Holiday.objects.filter(
        holiday_request__employee__exact=self,
        holiday_request__status__exact=1,
        date__gte=yearstart,
        date__lt=yearend
    ).distinct()
    return qs
...
def register(cls, admin_cls):
    ...
    cls.add_to_class('start_date', models.DateField(_('start date'), default=datetime.datetime.now))
